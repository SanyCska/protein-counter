...
`ai.parse_supplement_label` — состав банки с добавками. Оба промпта отдают данные на 100 г
и на суточный приём соответственно, картинку шлём с `detail: high` — на «low» мелкий шрифт
таблицы не читается. Вещества вне справочника (коллаген, пробиотики) возвращаются
