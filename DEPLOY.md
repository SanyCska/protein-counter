...
| `WEBAPP_URL` | HTTPS-адрес мини-аппа. Без него бот работает как раньше, просто без кнопки |
| `WEBAPP_ORIGINS` | Origin'ы, которым API разрешает запросы (обычно тот же адрес) |

## Мини-апп

`docker compose` поднимает два сервиса из одного образа: `bot` (polling) и `api` (uvicorn на
порту 8000). База — общий volume `bot-data`, так что блюда из бота сразу видны в мини-аппе.

Что нужно на сервере, чтобы мини-апп заработал:

1. **HTTPS перед API.** Telegram не открывает мини-аппы по http. Поставьте перед портом 8000
   любой обратный прокси с сертификатом (Caddy, nginx + certbot, Cloudflare Tunnel).
2. **Собранный фронтенд.** В репозитории `protein_web`: `npm ci && npm run build`, содержимое
   `dist/` раздаётся статикой. Если фронт и API на одном домене, `/api` проксируется на 8000
   и `VITE_API_BASE` менять не нужно.
3. **Секреты `WEBAPP_URL` и `WEBAPP_ORIGINS`** — адрес фронтенда. После деплоя бот сам
   поставит кнопку меню и команду `/app`.

Порт 8000 публикуется наружу в `docker-compose.yml`. Если прокси живёт на том же хосте,
безопаснее убрать блок `ports` у сервиса `api` и ходить к нему по имени сервиса из общей
docker-сети.
